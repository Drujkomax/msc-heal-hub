...
const ExecutiveOverview = () => {
  ...
  const getExecutiveMetrics = () => {
    ...
    const convertedLeads = leads.filter(lead => 
      lead.stage === 'converted' || lead.stage === 'closed'
    ).length;
  ...
  const getCriticalAlerts = () => {
    ...
    if (metrics.conversionRate < 2) {
      alerts.push({
        type: 'warning',
        message: 'Низкая конверсия товаров',
        value: `${metrics.conversionRate.toFixed(1)}%`,
        action: 'Требуется оптимизация каталога'
      });
    }
    ...
    if (metrics.leadConversionRate < 10) {
      alerts.push({
        type: 'danger',
        message: 'Низкая конверсия лидов',
        value: `${metrics.leadConversionRate.toFixed(1)}%`,
        action: 'Необходимо улучшить работу с лидами'
      });
    }
    ...
    if (metrics.avgActionsPerEmployee < 5) {
      alerts.push({
        type: 'info',
        message: 'Низкая активность сотрудников',
        value: `${metrics.avgActionsPerEmployee.toFixed(1)} действий/день`,
        action: 'Проверить загруженность персонала'
      });
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Сводная аналитика для принятия решений</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Конверсия товаров</CardTitle>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Активность команды</CardTitle>
      ...
      <Tabs defaultValue="products" className="space-y-4">
        <TabsList className="grid w-full grid-cols-3">
          ...
          <TabsTrigger value="leads">Лиды и сделки</TabsTrigger>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-muted-foreground">Активных товаров</span>
                  <span className="font-medium">{metrics.activeProducts}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-muted-foreground">Высокая конверсия</span>
                  <span className="font-medium">{metrics.topPerformingProducts}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-muted-foreground">Общие просмотры</span>
                  <span className="font-medium">{metrics.totalViews.toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-muted-foreground">Запросы КП</span>
        ...
        <TabsContent value="leads" className="space-y-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-muted-foreground">Всего лидов</span>
                  <span className="font-medium">{metrics.totalLeads}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-muted-foreground">Новые лиды</span>
            ...
            <Card>
              ...
              <CardContent>
                {/* Здесь можно добавить график распределения лидов по этапам */}
        ...
        <TabsContent value="team" className="space-y-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-muted-foreground">Активных сотрудников</span>
                ...
                <div className="flex justify-between items-center">
                  <span className="text-sm text-muted-foreground">Среднее на человека</span>
                ...
                <div className="space-y-3 text-sm">
                  {metrics.conversionRate < 2 && (
                    <div className="p-2 bg-orange-50 border border-orange-200 rounded">
                      <p className="font-medium text-orange-800">Оптимизация каталога</p>
                      <p className="text-orange-700">Улучшить описания и изображения товаров</p>
                    </div>
                  )}
                  {metrics.leadConversionRate < 10 && (
                    <div className="p-2 bg-red-50 border border-red-200 rounded">
                      <p className="font-medium text-red-800">Работа с лидами</p>
                      <p className="text-red-700">Ускорить обработку и follow-up</p>
                    </div>
                  )}
                  {metrics.avgActionsPerEmployee < 5 && (
                    <div className="p-2 bg-blue-50 border border-blue-200 rounded">
                      <p className="font-medium text-blue-800">Мотивация команды</p>
                      <p className="text-blue-700">Проверить загруженность и процессы</p>
                    </div>
                  )}
                </div>
              </CardContent>
